...
window.BMAP_AUTHENTIC_KEY = "OuBXloT9SmmhC8RdlGDbYUCfKc1NNyZ7";
(function () {
	function ba(a) {
		throw a;
	}
	...
	x.K.sj = function (a, b) {
		...
		d = a.style[b];
		if (!d)
			var e = c.FB[b], d = a.currentStyle || (x.da.la ? a.style : getComputedStyle(a, p)), d = e && e.get ? e.get(a, d) : d[e || b];
		if (e = c.Nl)
			d = e.filter(b, d, "get");
		return d
	};
	...
	function oa(a, b) {
		if (a.indexOf(offlineSourceUrl) < 0)  return;//修改  屏蔽ak验证，若调用外部资源直接返回
		if (b) {
...
// console.log(a.join(','));
							if( a.length > 0 ){
							  for(i=0; i<a.length;i++){
								mf = offlineSourceUrl+'modules/'+a[i]+'.js';
								oa( mf );
								//console.log(mf)
